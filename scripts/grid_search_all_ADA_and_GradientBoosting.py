...
print(grid_search.best_params_)
best_grid = grid_search.best_estimator_
evaluation(best_grid, 'best_grid_ada_treebase', X_test, y_test)

base_model = AdaBoostClassifier(base_estimator=rf)
base_model.fit(X_train_mini, y_train_mini)

evaluation(base_model, 'default_ada_treebase', X_test, y_test)
#%%
import pickle
# save the classifier
with open('best_ADA_treebase_classifier.pkl', 'wb') as fid:
    pickle.dump(best_grid, fid)
...
evaluation(base_model, 'default_gradientboost', X_test, y_test)
#%%
import pickle
# save the classifier
with open('best_gradientboost_classifier.pkl', 'wb') as fid:
    pickle.dump(best_grid, fid)
